...
Prisma maps all 55 legacy tables plus the NestJS-owned editorial tables. The full baseline migration
can initialize an empty database; on an existing database, record it with `./scripts/api.sh
baseline-existing` before deploying later migrations. Do not use `prisma migrate reset` or `prisma
db push` on an existing environment. Future schema changes must be committed as reviewed Prisma
migrations.
