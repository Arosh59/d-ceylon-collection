...
NestJS owns password, Google token exchange, application JWT, refresh-session, role, permission, and
ownership checks. Copy the JWT, Firebase Admin, SMTP, and reset URL settings from `.env.example`;
...
every committed operation is implemented, merges newly generated operations, and serves the result
at `/openapi/v1.json`.
