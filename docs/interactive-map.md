...
Set `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` in the public web deployment to show an interactive Google
Map with pins for supported published destinations. The key is necessarily browser-visible, so it
must be restricted to the production and development site HTTP referrers. Enable only the Maps
JavaScript API for this key and keep its billing, quota, and restriction settings under the
organisation's Google Cloud account. The integration uses Google Maps' weekly channel, Sri Lanka
region and English language settings, and sends only the configured catalogue coordinates to the
browser. It does not use Places, Geocoding, Directions, or live availability services.
...
`apps/web/public/images/hill-country-hero.png` is original, generated editorial artwork used only
as the public-home hero. It has no depicted people, logos, text, or third-party stock licence
claim. Product and destination-specific imagery remains owned by the editorial workflow and needs
rights metadata before publication.
